...
#define Q_NEW_HEAD(Q_HEAD_TYPE, Q_ELEM_TYPE)	\
  typedef struct Q_HEAD_TYPE##_struct {		\
    struct Q_ELEM_TYPE * first;			\
...
#define Q_INSERT_FRONT(Q_HEAD, Q_ELEM, LINK_NAME) \
  do {						  \
    if ((Q_HEAD)->first != NULL) {		  \
      (Q_HEAD)->first->LINK_NAME.prev = (Q_ELEM); \
    }						  \
    Q_INIT_ELEM(Q_ELEM, LINK_NAME);		  \
    (Q_ELEM)->LINK_NAME.next = (Q_HEAD)->first;	  \
    (Q_HEAD)->first = (Q_ELEM);			  \
    if ((Q_HEAD)->last == NULL) {		  \
      (Q_HEAD)->last = (Q_ELEM);		  \
    }						  \
 ...
 *          is empty
 **/
#define Q_GET_FRONT(Q_HEAD)   ((Q_HEAD)->first)
...
#define Q_REMOVE(Q_HEAD,Q_ELEM,LINK_NAME)		\
    do {						\
      if ((Q_HEAD)->first == (Q_ELEM)) {		\
	(Q_HEAD)->first = (Q_ELEM)->LINK_NAME.next;	\
      }							\
      if ((Q_HEAD)->last == (Q_ELEM)) {			\
...
#define Q_FOREACH(CURRENT_ELEM,Q_HEAD,LINK_NAME) \
	for (CURRENT_ELEM = (Q_HEAD)->first; CURRENT_ELEM != NULL; \
	     CURRENT_ELEM = CURRENT_ELEM->LINK_NAME.next)

#define Q_FOREACH_REV (CURRENT_ELEM,Q_HEAD,LINK_NAME) \
	for (CURRENT_ELEM = (Q_HEAD)->last; CURRENT_ELEM != NULL; \
